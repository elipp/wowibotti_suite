...
vec2_t screen2world(float x, float y, const arena_t &arena) {
...
vec2_t world2screen(float x, float y, const arena_t &arena) {
...
inline vec2_t world2screen(const vec2_t &v, const arena_t &arena) {
...
vec2_t tex2world(int x, int y, const arena_t &arena) {
...
hotness_status get_current_hotness_status() {
	...
	const auto config = get_active_hconfig();
	if (!config) { return m; }
...
int hconfig_set(const std::string &name) {
	for (const auto &c : hconfigs) {
		if (name == c.bossname) {
			ACTIVE_HCONFIG = name;
			dual_echo("hconfig_set: set hconfig to %s", name.c_str());
			return 1;
		}
	}
	dual_echo("hconfig_set: no hconfig with name %s found", name.c_str());
...
const hotness_config_t *get_active_hconfig() {
	...
	if (ACTIVE_HCONFIG == "") return NULL;
	if (!HOTNESS_ENABLED) return NULL;

	for (const auto &c : hconfigs) {
		if (ACTIVE_HCONFIG == c.bossname) {
			return &c;
		}
	}
...
int execute_current_hconfig_if_active() {
	...
	const auto c = get_active_hconfig();
	if (!c) return 0;
