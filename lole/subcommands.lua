...
function lole_set(attrib_name, on_off_str)
    if (attrib_name == nil or attrib_name == "") then
        echo("lole_set: no argument! valid modes for config " ..
            get_current_config().name .. " are: |cFFFFFF00" .. get_available_mode_attribs());
        return false;
    end
...
local function change_healer_targets(op, ...)
    local usage =
    "change_healer_targets: Usage: healer [set/add/del] healername [heals/hots/ignores] target1,target2,...,targetN [heals/hots/ignores] *targets ... where target = unitname (or 'raid' for heals option)";
